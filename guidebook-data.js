                    ...
                    },
                    {
                     ...
                     },
                     {
                         ...
                         "sections":  [
                                          ...
                                          },
                                          {
                                              ...
                                              "steps":  [
                                                              ...
                                                              {
                                                                  "title":  "완성된 스킬을 추가하고 테스트합니다",
                                                                  "body":  "요청을 보내면 Manus가 입력 자료의 범위, 빠진 정보의 처리 방식, 결과 구성처럼 스킬을 정확히 만들기 위해 필요한 내용을 질문할 수 있습니다. 질문에 답해 조건을 확정하면 Manus가 스킬을 만들고 점검한 뒤 결과 설명, SKILL.md 링크와 스킬 카드를 보여 줍니다. 카드의 ‘내 스킬에 추가’를 눌러 바로 사용할 수 있으며, SKILL.md 링크를 열어 내용을 확인하거나 파일로 내려받아 별도로 보관할 수도 있습니다. 화면 아래에 제시되는 연관 질문을 선택하면 샘플 자료로 스킬을 시험하거나, SKILL.md 전체 내용 확인, 분류 항목 추가처럼 이어서 수정할 수 있습니다. 실제 업무에 사용하기 전에는 한두 개의 샘플로 결과를 확인하고 부족한 규칙을 보완합니다.",
                                                                  "image":  "./guidebook-assets/manus_skills_result.png",
                                                                  "badge":  "저장·테스트"
                                                              }
                                                          ]
                                          },
                                          ...
                                          },
                                          {
                                              ...
                                              "title":  "웹사이트 구축",
                                              "image":  "./guidebook-assets/manus_feature_menu.png",
                                              "short":  "원하는 사이트나 웹앱을 말로 설명하면 Manus가 화면 디자인부터 프런트엔드·백엔드·데이터베이스까지 만들고 미리보기와 게시 환경을 제공합니다.",
                                              "detail":  "자료·참고 사이트·Figma 디자인을 바탕으로 제작 조건을 정하고, 생성된 웹사이트를 데스크톱과 모바일에서 시험한 뒤 대화나 직접 편집으로 수정하고 게시하는 방법을 안내합니다.",
                                              "when":  "랜딩 페이지·회사 소개·포트폴리오처럼 정보를 보여 주는 사이트부터 로그인, 입력 폼, 데이터 저장, 대시보드, 예약·결제 등이 필요한 웹앱의 초안이나 실제 서비스를 빠르게 만들 때 활용합니다.",
                                              "steps":  [
                                                              {
                                                                  "title":  "메인 화면에서 웹사이트 구축을 엽니다",
                                                                  "body":  "메인 입력창 아래의 ‘웹사이트 구축’을 누르면 웹사이트 모드가 활성화됩니다. 이 기능은 화면 모양만 만드는 도구가 아니라, 자연어 요청을 바탕으로 사이트의 코드와 필요한 서버 기능까지 구성하는 대화형 웹 개발 도구입니다. 간단한 랜딩 페이지·회사 소개·포트폴리오뿐 아니라 대시보드, 회원 로그인, 상품·예약 관리, 데이터 입력 폼 같은 동작하는 웹앱도 요청할 수 있습니다. 다만 데이터베이스·로그인·결제·외부 API 같은 기능은 요청 내용에 포함해야 하며, 실제 운영 전에는 권한·보안·결제 테스트와 데이터 정확성을 별도로 확인합니다.",
                                                                  "image":  "./guidebook-assets/manus_feature_menu.png",
                                                                  "badge":  "기능 열기"
                                                              },
                                                              {
                                                                  "title":  "자료와 디자인을 연결하고 만들 사이트를 구체화합니다",
                                                                  "body":  "웹사이트가 활성화된 입력창에서 ‘+’를 눌러 컴퓨터 파일, 라이브러리 자료, 이전 작업, Drive 등의 참고 자료를 첨부할 수 있습니다. ‘웹사이트 참조 추가’에는 참고하고 싶은 기존 사이트 주소를 넣고, ‘Figma에서 가져오기’에는 미리 만든 Figma 프레임을 연결합니다. Manus는 Figma의 레이아웃·컴포넌트·글꼴·색상을 읽어 작동하는 웹 화면의 코드로 옮길 수 있으며, 여러 프레임을 가져올 때는 각 프레임의 화면 순서와 클릭·호버 같은 상태 관계도 함께 설명하는 것이 좋습니다. 아래의 전자상거래·랜딩 페이지·대시보드·포트폴리오·기업 등의 유형은 시작 방향을 빠르게 정하는 선택지이고, 반드시 그 범위에만 제한되지는 않습니다. 입력창에는 사이트 목적, 대상 사용자, 필요한 페이지, 핵심 기능, 저장할 데이터, 로그인·결제 여부, 원하는 색상과 분위기, 모바일 대응 여부를 적습니다. 예: ‘국내외 주식 보유 현황을 보여 주는 반응형 대시보드를 만들어 줘. 종목 추가·수정, 평가금액·손익 계산, 뉴스 모음을 넣고 우선 샘플 데이터로 작동하게 해 줘.’",
                                                                  "image":  "./guidebook-assets/manus_website_setup.png",
                                                                  "badge":  "자료·요구사항"
                                                              },
                                                              {
                                                                  "title":  "미리보기에서 기능을 시험하고 수정한 뒤 게시합니다",
                                                                  "body":  "생성이 끝나면 왼쪽에는 작업 결과와 후속 요청 입력창, 오른쪽에는 실제로 조작할 수 있는 실시간 미리보기가 나타납니다. 미리보기 위의 데스크톱·모바일 버튼으로 반응형 화면을 비교하고, 경로 표시줄로 현재 페이지 주소를 확인하며 새로고침이나 전체 화면으로 동작을 점검합니다. 상단의 코드 영역에서는 생성된 프런트엔드·백엔드 파일을 확인하거나 전체 코드를 내려받을 수 있습니다. 데이터베이스를 사용한 사이트라면 데이터 영역에서 테이블과 레코드를 검색·추가·수정하고 구조를 확인할 수 있으며, 파일 영역은 업로드 이미지·문서 같은 저장 자료를 관리하는 데 사용합니다. 표시되는 도구는 해당 사이트에 실제로 생성된 기능에 따라 달라질 수 있습니다. ‘편집’에서는 요소를 선택해 색상·글꼴·간격·테두리를 직접 조정할 수 있고, 왼쪽 대화창에 ‘모바일 메뉴를 아래쪽으로 바꿔 줘’, ‘종목 검색과 알림을 추가해 줘’처럼 말해 큰 변경을 맡길 수도 있습니다. 충분히 시험한 뒤 ‘게시’를 누르면 공개 주소로 배포되며, 필요하면 사용자 도메인을 연결할 수 있습니다. 게시 후 수정 사항도 다시 배포할 수 있으므로 실제 고객 데이터나 결제 연결 전에는 샘플 데이터와 테스트 계정으로 먼저 검증합니다.",
                                                                  "image":  "./guidebook-assets/manus_website_result.png",
                                                                  "badge":  "미리보기·게시"
                                                              }
                                                          ]
                                          },
                                          ...
                                          },
                                          {
                                              ...
                                              "title":  "게임 제작",
                                              "image":  "./guidebook-assets/manus_feature_menu.png",
                                              "short":  "만들고 싶은 게임을 자연어로 설명하면 Manus가 브라우저에서 실행되는 게임의 화면·규칙·조작과 코드를 구성하고, 직접 플레이하며 수정·게시할 수 있게 합니다.",
                                              "detail":  "게임 제작 모드를 열어 장르·조작법·승패 조건·디자인을 프롬프트로 전달하고, 생성된 게임을 실시간 미리보기에서 시험한 뒤 기능을 보완하고 공개하는 방법을 안내합니다.",
                                              "when":  "교육용 퀴즈, 퍼즐, 카드·보드 게임, 간단한 아케이드, 시뮬레이션, 프로모션용 미니게임부터 2D·3D 브라우저 게임의 아이디어와 작동 가능한 시제품을 빠르게 만들 때 활용합니다.",
                                              "steps":  [
                                                              {
                                                                  "title":  "메인 화면에서 게임 제작을 엽니다",
                                                                  "body":  "메인 입력창 아래의 ‘게임 제작’을 누르면 게임 제작 모드가 활성화됩니다. 이 기능은 자연어 요청을 바탕으로 브라우저에서 실행되는 웹게임의 화면, 규칙, 조작과 코드를 한 번에 만드는 전용 시작점입니다. 반응 속도 게임, 기억 맞추기, 뱀 게임, 카드·보드·퀴즈처럼 작은 게임뿐 아니라 여러 캐릭터·스킬·레벨과 2D 또는 3D 화면이 있는 시제품도 요청할 수 있습니다. 다만 Unity나 Unreal 같은 전문 게임 엔진 프로젝트를 직접 편집하는 화면은 아니며, 고사양 그래픽·실시간 멀티플레이·결제·계정 저장처럼 복잡한 기능은 별도의 서버 구성과 충분한 테스트가 필요합니다.",
                                                                  "image":  "./guidebook-assets/manus_feature_menu.png",
                                                                  "badge":  "기능 열기"
                                                              },
                                                              {
                                                                  "title":  "원하는 게임을 프롬프트로 설명하고 생성합니다",
                                                                  "body":  "게임 제작이 활성화된 입력창에 요청을 적고 전송하면 됩니다. 짧은 요청만으로도 시작할 수 있고, 화면 아래의 반딧불 기억 순서·사격·뱀·타로·4목 같은 예시를 눌러 기본 프롬프트를 불러올 수도 있습니다. 완성도를 높이려면 장르와 세계관, 플레이 목표, 키보드·마우스·터치 조작, 시작·일시정지·재시작 방법, 점수와 승패 조건, 난이도 변화, 화면 크기, 음향 여부와 꼭 필요한 화면을 함께 적습니다. 예: ‘PC와 모바일 브라우저에서 실행되는 2D MOBA 시제품을 만들어 줘. 클릭 이동과 Q·W·E·R 스킬, 체력·마나, 적 AI, 미니맵, 상점, 승패 조건을 넣고 먼저 한 개 전장과 두 챔피언으로 테스트하게 해 줘.’ 이미지·사운드·기획서를 가지고 있다면 ‘+’로 첨부해 참고시키며, 타인의 캐릭터·음악·그래픽을 그대로 복제하지 않도록 사용 권한을 확인합니다.",
                                                                  "image":  "./guidebook-assets/manus_game_prompt.png",
                                                                  "badge":  "프롬프트 생성"
                                                              },
                                                              {
                                                                  "title":  "직접 플레이해 수정하고 게시합니다",
                                                                  "body":  "작업이 끝나면 왼쪽에는 결과 카드와 후속 요청 입력창, 오른쪽에는 실제로 조작할 수 있는 게임 미리보기가 나타납니다. 데스크톱·모바일 전환으로 화면과 터치 대응을 확인하고, 이동·공격·스킬·충돌·점수·승패·재시작을 여러 번 시험합니다. 문제가 있으면 ‘적 AI가 벽을 통과하지 않게 해 줘’, ‘모바일용 가상 조이스틱을 추가해 줘’, ‘초반 난이도를 낮추고 보스전을 추가해 줘’처럼 재현 조건과 원하는 동작을 구체적으로 요청합니다. 코드·파일·설정 영역에서는 생성된 프로젝트 구성과 에셋을 확인할 수 있고, 필요하면 기능을 추가하거나 이전 변경 상태를 검토할 수 있습니다. ‘게시’를 누르면 공유 가능한 웹 주소로 배포하고 이후 수정본도 다시 게시할 수 있습니다. 공개 전에는 모바일과 PC에서 프레임 속도, 로딩, 조작 설명, 음량, 저장 초기화와 개인정보·결제·외부 API를 모두 점검합니다. 생성형 AI 결과는 작동하더라도 게임 밸런스와 재미가 자동으로 보장되지는 않으므로 실제 이용자 테스트를 거쳐 다듬습니다.",
                                                                  "image":  "./guidebook-assets/manus_game_result.png",
                                                                  "badge":  "테스트·게시"
                                                              }
                                                          ]
                                          },
                         ...
                         "sources":  [
                                         ...
                                         "https://www.manus.im/features/webapp",
                                         ...
                                         "https://manus.im/",
                                         "https://academy.manus.im/projects/019b38b5-a80e-753a-bf1a-d5ef1a831559"
                                     ]
                     },
                     {
                         "id":  "elevenlabs",
